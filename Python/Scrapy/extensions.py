...
# -------------------------------------------------------------------------------
# Name         :  extensions.py
# Description  :  用于Scrapy-redis在队列空后一段时间后直接结束爬虫
...
class RedisSpiderSmartIdleClosedExtensions(object):
    ...
    def from_crawler(cls, crawler):
        # 首先检查是否应该启用和提高扩展
        # 否则不配置
        if not crawler.settings.getbool('REDISEXT_ENABLED'):
            raise NotConfigured

        # 获取配置中的时间片个数，默认为360个，每个是5秒钟
    ...
    def spider_opened(self, spider):
        logger.info("opened spider %s redis spider Idle, Continuous idle limit： %d",
    ...
    def spider_idle(self, spider):
        ...
        if idle_list_len > 2 and self.idle_list[-1] - self.idle_list[-2] > 6:
            ...
            # 连续触发的次数达到配置次数后关闭爬虫
            start_time_int = time.localtime(self.idle_list[0])
            start_time_str = time.strftime("%Y-%m-%d %H:%M:%S", start_time_int)

            end_time_int = time.localtime(self.idle_list[1])
            end_time_str = time.strftime("%Y-%m-%d %H:%M:%S", end_time_int)
